...
gcor_list = torch.zeros(len(sample_list),len(nodes_list))
gcov_list = torch.zeros(len(sample_list),len(nodes_list))

for l in range(len(sample_list)):
    sample = sample_list[l]
    for m in range(len(nodes_list)):
        ...
        for h in range(10):
            ...
            gcov_value.append(gcov)

            g1 = U_product_gpu(tilde_p1,tilde_p1)
            g2 = U_product_gpu(tilde_p2,tilde_p2)
            gcorr = gcov/torch.sqrt(g1*g2)
            gcor_value.append(gcorr)
        gcov_list[l,m] = np.mean(gcov_value)

        gcor_list[l,m] = np.mean(gcor_value)

        print(l,m,np.mean(gcov_value),np.mean(gcor_value))
...
df2.to_csv('/public3/home/scg5453/wenxy/tcor_cubic_mat.csv')
